...
__export(index_exports, {
  ...
  getFasterMedian: () => getFasterMedian,
  getFasterQuartile: () => getFasterQuartile,
...
var import_big30 = __toESM(require("big.js"), 1);
...
function getFasterMedian(values) {
  const n = values.length;
  if (n === 0) {
    throw new Error("Cannot calculate median of empty array");
  }
...
function getFasterQuartile(values, q) {
...
var IQR = class extends BigIndicatorSeries {
  constructor(interval) {
    super();
    this.interval = interval;
  }
  values = [];
  update(value, replace) {
    const bigValue = new import_big17.default(value);
    if (replace) {
      this.values.pop();
    }
...
var FasterIQR = class extends NumberIndicatorSeries {
  constructor(interval) {
    super();
    this.interval = interval;
  }
...
var LinearRegression = class extends TechnicalIndicator {
  ...
  calculateRegression(prices) {
    const n = new import_big18.default(prices.length);
    const xValues = Array.from({ length: prices.length }, (_, i) => new import_big18.default(i));
    const yValues = prices.map((p) => new import_big18.default(p));
    const sumX = xValues.reduce((sum, x) => sum.add(x), new import_big18.default(0));
    const sumY = yValues.reduce((sum, y) => sum.add(y), new import_big18.default(0));
    const sumXY = xValues.reduce((sum, x, i) => sum.add(x.mul(yValues[i])), new import_big18.default(0));
    const sumXX = xValues.reduce((sum, x) => sum.add(x.mul(x)), new import_big18.default(0));
...
var OBV = class extends BigIndicatorSeries {
  ...
  update(candle, replace) {
    ...
    const prevResult = this.result ?? new import_big20.default(0);
    const currentPrice = new import_big20.default(candle.close);
...
var PSAR = class extends BigIndicatorSeries {
  accelerationStep;
  accelerationMax;
  acceleration = new import_big21.default(0);
  extreme = null;
  lastSar = null;
  isLong = null;
  previousCandle = null;
  ...
  constructor(config) {
    super();
    this.accelerationStep = new import_big21.default(config.accelerationStep);
    this.accelerationMax = new import_big21.default(config.accelerationMax);
  ...
  update(candle, replace) {
    const high = new import_big21.default(candle.high);
    const low = new import_big21.default(candle.low);
    ...
    if (this.lastSar === null) {
      ...
      const previousMidpoint = new import_big21.default(this.previousCandle.high).add(this.previousCandle.low).div(2);
      this.isLong = currentMidpoint.gte(previousMidpoint);
      if (this.isLong) {
        this.extreme = high;
        this.lastSar = new import_big21.default(this.previousCandle.low);
      } else {
        this.extreme = low;
        this.lastSar = new import_big21.default(this.previousCandle.high);
      }
    ...
    if (this.isLong) {
      if (this.previousCandle) {
        const previousLow = new import_big21.default(this.previousCandle.low);
        if (this.prePreviousCandle && low.lt(sar)) {
          const prePreviousLow = new import_big21.default(this.prePreviousCandle.low);
      ...
      if (this.previousCandle) {
        const previousHigh = new import_big21.default(this.previousCandle.high);
        const hasPrevPrev = this.prePreviousCandle != null;
        if (hasPrevPrev && high.gt(sar)) {
          const prePreviousHigh = new import_big21.default(this.prePreviousCandle.high);
...
var FasterPSAR = class extends NumberIndicatorSeries {
  accelerationStep;
  accelerationMax;
  acceleration = 0;
  extreme = null;
...
// src/RMA/RMA.ts
var import_big22 = __toESM(require("big.js"), 1);
var RMA = class extends MovingAverage {
  ...
  update(_price, replace) {
    ...
    const price = new import_big22.default(_price);
    if (replace && this.previousResult) {
      return this.setResult(
        price.times(this.weightFactor).add(this.previousResult.times(1 - this.weightFactor)),
        replace
...
// src/ROC/ROC.ts
var import_big23 = __toESM(require("big.js"), 1);
var ROC = class extends BigIndicatorSeries {
  ...
  update(price, replace) {
    ...
    if (comparePrice) {
      return this.setResult(new import_big23.default(price).sub(comparePrice).div(comparePrice), replace);
    }
    return null;
  }
};
...
// src/RSI/RSI.ts
var import_big24 = __toESM(require("big.js"), 1);
var RSI = class extends BigIndicatorSeries {
  ...
  avgGain;
  avgLoss;
  maxValue = new import_big24.default(100);
  update(price, replace) {
    pushUpdate(this.previousPrices, replace, price, this.interval);
    if (this.previousPrices.length < 2) {
      return null;
    }
    const currentPrice = new import_big24.default(price);
    const previousPrice = new import_big24.default(this.previousPrices[this.previousPrices.length - 2]);
    if (currentPrice.gt(previousPrice)) {
      this.avgLoss.update(new import_big24.default(0), replace);
      this.avgGain.update(currentPrice.sub(previousPrice), replace);
    } else {
      this.avgLoss.update(previousPrice.sub(currentPrice), replace);
      this.avgGain.update(new import_big24.default(0), replace);
    }
    if (this.avgGain.isStable) {
      const avgLoss = this.avgLoss.getResultOrThrow();
      if (avgLoss.eq(0)) {
        return this.setResult(new import_big24.default(100), replace);
      }
...
var import_big25 = __toESM(require("big.js"), 1);
var StochasticOscillator = class extends TechnicalIndicator {
  ...
  update(candle, replace) {
    ...
    if (this.candles.length === this.n) {
      ...
      const divisor = new import_big25.default(highest).minus(lowest);
      let fastK = new import_big25.default(100).mul(new import_big25.default(candle.close).minus(lowest));
  ...
  update(price, replace) {
    ...
    if (rsiResult) {
      ...
      if (periodResult) {
        ...
        if (denominator.eq(0)) {
          return this.setResult(new import_big26.default(100), replace);
        }
...
var FasterStochasticRSI = class extends NumberIndicatorSeries {
  constructor(interval, SmoothingRSI = FasterWSMA, smoothing = {
    d: new FasterSMA(3),
    k: new FasterSMA(3)
  }) {
    super();
...
// src/TDS/TDS.ts
var import_big27 = __toESM(require("big.js"), 1);
var TDS = class extends BigIndicatorSeries {
  closes = [];
  setupCount = 0;
  setupDirection = null;
  update(close, replace) {
    const closeBig = new import_big27.default(close);
    if (replace) {
      this.closes.pop();
    }
    ...
    if (this.setupCount >= 9) {
      const result = new import_big27.default(this.setupDirection === "bullish" ? 1 : -1);
      this.setupCount = 0;
...
var FasterTDS = class extends NumberIndicatorSeries {
  closes = [];
  setupCount = 0;
  setupDirection = null;
  update(close, replace) {
...
// src/VWAP/VWAP.ts
var import_big28 = __toESM(require("big.js"), 1);
var VWAP = class extends BigIndicatorSeries {
  cumulativeTypicalPriceVolume = new import_big28.default(0);
  cumulativeVolume = new import_big28.default(0);
  lastCandle = null;
  constructor() {
    super();
  }
  calculateTypicalPriceVolume(data) {
    const high = new import_big28.default(data.high);
...
var FasterVWAP = class extends NumberIndicatorSeries {
...
// src/WMA/WMA.ts
var import_big29 = __toESM(require("big.js"), 1);
  ...
  update(price, replace) {
    ...
    if (this.prices.length === this.interval) {
      const weightedPricesSum = this.prices.reduce((acc, price2, index) => {
        const weightedPrice = new import_big29.default(price2).mul(index + 1);
        return acc.add(weightedPrice);
      }, new import_big29.default(0));
