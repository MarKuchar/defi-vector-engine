...
var IQR = class extends BigIndicatorSeries {
  ...
  values = [];
  update(value, replace) {
    const bigValue = new Big17(value);
    if (replace) {
      this.values.pop();
    }
...
var LinearRegression = class extends TechnicalIndicator {
  ...
  calculateRegression(prices) {
    const n = new Big18(prices.length);
    const xValues = Array.from({ length: prices.length }, (_, i) => new Big18(i));
    const yValues = prices.map((p) => new Big18(p));
    const sumX = xValues.reduce((sum, x) => sum.add(x), new Big18(0));
    const sumY = yValues.reduce((sum, y) => sum.add(y), new Big18(0));
    const sumXY = xValues.reduce((sum, x, i) => sum.add(x.mul(yValues[i])), new Big18(0));
    const sumXX = xValues.reduce((sum, x) => sum.add(x.mul(x)), new Big18(0));
...
var OBV = class extends BigIndicatorSeries {
  ...
  update(candle, replace) {
    ...
    const prevResult = this.result ?? new Big20(0);
    const currentPrice = new Big20(candle.close);
...
var PSAR = class extends BigIndicatorSeries {
  ...
  constructor(config) {
    super();
    this.accelerationStep = new Big21(config.accelerationStep);
    this.accelerationMax = new Big21(config.accelerationMax);
  ...
  update(candle, replace) {
    const high = new Big21(candle.high);
    const low = new Big21(candle.low);
    ...
    if (this.lastSar === null) {
      ...
      const previousMidpoint = new Big21(this.previousCandle.high).add(this.previousCandle.low).div(2);
      this.isLong = currentMidpoint.gte(previousMidpoint);
      if (this.isLong) {
        this.extreme = high;
        this.lastSar = new Big21(this.previousCandle.low);
      } else {
        this.extreme = low;
        this.lastSar = new Big21(this.previousCandle.high);
      }
    ...
    if (this.isLong) {
      if (this.previousCandle) {
        const previousLow = new Big21(this.previousCandle.low);
        if (this.prePreviousCandle && low.lt(sar)) {
          const prePreviousLow = new Big21(this.prePreviousCandle.low);
      ...
      if (this.previousCandle) {
        const previousHigh = new Big21(this.previousCandle.high);
        const hasPrevPrev = this.prePreviousCandle != null;
        if (hasPrevPrev && high.gt(sar)) {
          const prePreviousHigh = new Big21(this.prePreviousCandle.high);
...
var ROC = class extends BigIndicatorSeries {
  ...
  update(price, replace) {
    ...
    if (comparePrice) {
      return this.setResult(new Big23(price).sub(comparePrice).div(comparePrice), replace);
    }
    return null;
  }
};
...
var StochasticOscillator = class extends TechnicalIndicator {
  ...
  update(candle, replace) {
    ...
    if (this.candles.length === this.n) {
      ...
      const divisor = new Big25(highest).minus(lowest);
      let fastK = new Big25(100).mul(new Big25(candle.close).minus(lowest));
  ...
  update(price, replace) {
    ...
    if (rsiResult) {
      ...
      if (periodResult) {
        ...
        if (denominator.eq(0)) {
          return this.setResult(new Big26(100), replace);
        }
...
var TDS = class extends BigIndicatorSeries {
  ...
  update(close, replace) {
    ...
    if (this.setupCount >= 9) {
      const result = new Big27(this.setupDirection === "bullish" ? 1 : -1);
      this.setupCount = 0;
...
// src/VWAP/VWAP.ts
import Big28 from "big.js";
var VWAP = class extends BigIndicatorSeries {
  cumulativeTypicalPriceVolume = new Big28(0);
  cumulativeVolume = new Big28(0);
  lastCandle = null;
  constructor() {
    super();
  }
  calculateTypicalPriceVolume(data) {
    const high = new Big28(data.high);
  ...
  update(price, replace) {
    ...
    if (this.prices.length === this.interval) {
      const weightedPricesSum = this.prices.reduce((acc, price2, index) => {
        const weightedPrice = new Big29(price2).mul(index + 1);
